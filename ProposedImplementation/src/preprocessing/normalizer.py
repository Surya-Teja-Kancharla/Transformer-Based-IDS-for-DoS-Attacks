...
Phase 2 notes:
  - Normalization logic is IDENTICAL to Phase 1 — same Eq.(2) applied
    after CTGAN-based augmentation (Phase 2) instead of SMOTE-ENN (Phase 1).
  - Normalization is applied BEFORE CTGAN training, which means CTGAN
    learns to generate data already in the [0, 1] normalised space.
    This avoids the need to re-normalise synthetic samples post-generation.
  - Fit ONLY on training split (never on validation/test) — unchanged.
  - NaN-safe: clips output to [0, 1] even if test has values outside
    training range (common in network traffic) — unchanged.
  - Saves min/max arrays for reproducibility and fog/IoT deployment.
...
class MinMaxNormalizer:
    ...
    Phase 2 note:
      Output feeds CTGANBalancer (Phase 2) instead of DOSSMOTEENNHandler
      (Phase 1). CTGAN receives pre-normalised [0,1] features and generates
      synthetic samples in that same space, ensuring no re-normalisation step
      is needed after augmentation.
