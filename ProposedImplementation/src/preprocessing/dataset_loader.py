...
Phase 2 note:
  ...
  dataset_loader.py — data loading and cleaning logic is shared between
  phases. The only downstream difference is Step 5 of data_pipeline.py
  where CTGAN (Phase 2) replaces SMOTE-ENN (Phase 1).
...
class CICIDSDatasetLoader:
    ...
    max_benign_samples : int | None
        ...
        class imbalance before CTGAN. Set None to keep all.
    random_state : int
    ...
    def run_full_pipeline(
        ...
        -------
        df : pd.DataFrame
            Fully processed DataFrame ready for normalization/CTGAN.
        class_map : Dict[int, str]
    ...
    def _validate_path(self) -> None:
        if not self.data_path.exists():
            raise FileNotFoundError(
                ...
                "ProposedImplementation/data/raw/"
            )
