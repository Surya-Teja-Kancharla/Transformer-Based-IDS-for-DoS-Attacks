...
metrics.py
==========
Evaluation metrics for LightweightIDSModel (Phase 2) per paper Section 4.3.

Phase 2 twin of ExistingImplementation/src/evaluation/metrics.py.
Identical implementation -- metrics, plotting functions, and class name
constants are fully reusable between phases.

Paper Eqs. (9)-(12) -- same formulas apply for both phases:
...
WHAT THIS FILE PRODUCES (same as Phase 1):
  evaluate_model()         -> accuracy, precision, recall, F1 on test set
  plot_confusion_matrix()  -> PNG: normalised confusion matrix
  plot_per_class_metrics() -> PNG: per-class precision/recall/F1 bar chart
  plot_training_history()  -> PNG: train/val loss and accuracy curves
  save_metrics_json()      -> JSON: all scalar metrics saved to disk
...
def evaluate_model(
    ...
    Parameters
    ----------
    model      : trained LightweightIDSModel (or any dual-branch model)
