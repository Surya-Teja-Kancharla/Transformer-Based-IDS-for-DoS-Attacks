...
PIPELINE_CONFIG = {
    ...
    # SMOTE-ENN
    "smote_target_per_class": 15_000,  # 15K x 5 classes = 75K total (RTX 2050 safe)
    "smote_sampling_strategy": "fixed", # "fixed" = cap all classes at target_per_class
    "smote_k_neighbors": 5,
...
class DOSPreprocessingPipeline:
    ...
    def run(
        ...
        smote_handler = DOSSMOTEENNHandler(
            ...
            sampling_strategy=self.config.get("smote_sampling_strategy", "fixed"),
            random_state=self.config["random_state"],
            verbose=self.verbose,
        )
